...
class MainPanel(Frame):
    def __init__(self, parent, controller):
        ...
        self.balance.place(x=1300, y=380)
        ...
        self.withdrawl.bind("<Enter>", lambda id: self.withdrawl.place(x=1280))
        self.withdrawl.bind("<Leave>", lambda id: self.withdrawl.place(x=1300))
        self.deposit.bind("<Enter>", lambda id: self.deposit.place(x=1280))
        self.deposit.bind("<Leave>", lambda id: self.deposit.place(x=1300))
        self.balance.bind("<Enter>", lambda id: self.balance.place(x=1280))
        self.balance.bind("<Leave>", lambda id: self.balance.place(x=1300))
    ...
    def withdrawlFunc(self):
        ...
        con = pymysql.connect(host="localhost", user="root", password="root", database="fatm")
        cur = con.cursor()
...
class AdminPanel(Frame):
    def __init__(self, parent, controller):
        ...
        self.dobEntry.insert(0, "In form of 26/02/2001")
        self.dobEntry.bind("<Enter>", lambda id: self.dobEntry.delete(0,
                                                                      END) if self.dobEntry.get() == "In form of 26/02/2001" else "")

        self.dobEntry.bind("<Leave>", lambda id: self.dobEntry.insert(0,
                                                                      "In form of 26/02/2001" if self.dobEntry.get() == "" else ""))
        ...
        # aadhar
        aadharLbl = Label(Manage_Frame, text="Aadhar No :", font=("Comic Sans MS", 12), bg="black",
                          fg="white", pady=10)
        aadharLbl.grid(row=7, column=0)
        self.aadharEntry = Entry(Manage_Frame, font=("Comic Sans MS", 12), bg="white", fg="black")
        self.aadharEntry.grid(row=7, column=1, padx=10)
        ...
        'Aadhar No', 'Face Id','Pin','Balance'), xscrollcommand=scrollX.set, yscrollcommand=scrollY.set)
        ...
        self.dataTable.heading('Aadhar No', text="Aadhar No")
        ...
        self.dataTable.column('Aadhar No', width=150,anchor=CENTER)
    ...
    def fetchData(self):
        ...
        cur.execute("select accountNo,accountHolder,email,gender,dateOfBirth,address,contact,aadharNo,faceAdded,pin,balance from userdata")
        rows=cur.fetchall()
    ...
    def fetchDataWithCondition(self,column,value):
        ...
        if(column=="A/c Number"):
            column="accountNo"
            query= "select accountNo,accountHolder,email,gender,dateOfBirth,address,contact,aadharNo,faceAdded,pin,balance from userdata where " + column.lower() + " = " + value + ""

        elif(column=="Date of Birth"):
            column="dateOfBirth"
            query = "select accountNo,accountHolder,email,gender,dateOfBirth,address,contact,aadharNo,faceAdded,pin,balance from userdata where " + column.lower() + " like '%" + value + "%'"
    ...
    def generateAccNoFunc(self):
        ...
        self.aadhar = self.aadharEntry.get().strip()
        ...
        if (self.dateOfBirth == "" or self.dateOfBirth == "In form of 26/02/2001"):
    ...
    def setValues(self,row):
        ...
        self.aadharEntry.insert(0, row[7])
        self.faceEntry.insert(0, row[8])
        self.pinNoEntry.insert(0,row[9])
    ...
    def saveAllFunc(self):
        ...
        if (self.aadhar == ""):
            messagebox.showerror("Error", "Empty Aadhar No! Please fill it first")
            return
        else:
            regex = '^[0-9]{12}$'
            if ((re.search(regex, self.aadhar))):
                pass
            else:
                messagebox.showerror("Error", "Invalid Aadhar Number ! Please check it")
                return
        if (self.accountNo == ""):
            messagebox.showerror("Error", "Empty Account No! Generate Account Number First")
            return
        ...
        if(noOfRows==1):
            cur.execute("delete from userdata where accountNo=" + self.accountNoEntry.get().strip())
            con.commit()
        result=cur.execute("insert into Userdata values(%s,%s,%s,%s,%s,%s,%s,%s,%s,"+self.accountNo+",0)",(
        self.accountHolder,
        self.email,
        self.gender,
        self.dateOfBirth,
